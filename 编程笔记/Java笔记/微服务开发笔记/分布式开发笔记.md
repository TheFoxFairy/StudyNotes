...
**Zookeeper 是⼀种分布式协调服务，⽤于管理大型主机。**在分布式环境中协调和管理服务是⼀个复杂的过程。Zookeeper 通过其简单的架构和API解决了这个问题。Zookeeper允许开发⼈员专注于核⼼应⽤程序逻辑，⽽不必担⼼应⽤程序的分布式特性。
...
如果⽤上述的上锁⽅式，只要有节点发⽣变化，就会触发其他节点的监听事件，这样的话对zk的压⼒⾮常⼤，即⽺群效应。可以通过调整成链式监听，解决这个问题。
